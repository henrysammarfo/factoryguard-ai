...
export default function AnalyticsPage() {
  ...
  async function loadAnalyticsData() {
    try {
      // Mock equipment data for analytics
      const mockEquipment = [
        {
          id: "motor-001",
          ...
          temperature: 68,
          vibration: 2.1,
          latest_reading: [{ energy_consumption: 245 }]
        },
        {
          id: "pump-002",
          name: "Hydraulic Pump",
          ...
          temperature: 75,
          vibration: 3.8,
          latest_reading: [{ energy_consumption: 198 }]
        },
        {
          id: "fan-003",
          name: "Cooling Fan",
          ...
          temperature: 45,
          vibration: 1.2,
          latest_reading: [{ energy_consumption: 312 }]
        },
        {
          id: "sensor-004",
          ...
          temperature: 82,
          vibration: 0.5,
          latest_reading: [{ energy_consumption: 385 }]
        }
      ]
      ...
      const mockPredictions = [
        {
          ...
          risk_level: 'low',
          ai_insights: ["Normal operating conditions detected"]
        },
        {
          id: "pred-002",
          equipment_id: "pump-002",
          ...
          prediction_date: new Date().toISOString(),
          risk_level: 'medium',
          ai_insights: ["Vibration patterns indicate bearing degradation"]
        }
      ]

      // Convert alerts to anomalies format
      const formattedAnomalies = mockAlerts.filter((alert: any) => !alert.resolved).map((alert: any) => ({
        id: alert.id,
        anomaly_type: `${alert.sensor.charAt(0).toUpperCase() + alert.sensor.slice(1)} Alert`,
        severity: alert.type,
  ...
  function generateAIInsightsFromPrediction(prediction: any) {
    const insights = []

    // Get equipment data for insights
    const eq = equipment.find((e: any) => e.id === prediction.equipment_id)
    if (!eq) return ["Equipment data not available"]
    ...
    if (prediction.predicted_value < 30) {
