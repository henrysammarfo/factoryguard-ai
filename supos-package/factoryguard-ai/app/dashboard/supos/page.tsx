...
import { AlertCircle, CheckCircle, XCircle, RefreshCw, Send } from 'lucide-react'
...
export default function SupOSDashboard() {
  ...
  const testMQTTPublish = async () => {
    console.log('[FactoryGuard] 🧪 TESTING MQTT PUBLISH FUNCTION')
    try {
      const { publishToUNSTopic, connectToSupOSMQTT } = await import('@/lib/supos/client')

      console.log('[FactoryGuard] 🔌 Connecting to MQTT...')
      const connected = await connectToSupOSMQTT()
      if (!connected) {
        console.error('[FactoryGuard] ❌ MQTT connection failed')
        alert('MQTT connection failed - check console for details')
        return
      }

      console.log('[FactoryGuard] 📤 Publishing test message...')
      const testTopic = 'factory/test/demo'
      const testData = {
        message: 'FactoryGuard AI Test Message',
        timestamp: new Date().toISOString(),
        source: 'demo_test',
        type: 'bidirectional_flow_demo'
      }

      const success = publishToUNSTopic(testTopic, testData)
      if (success) {
        console.log('[FactoryGuard] ✅ TEST MESSAGE PUBLISHED!')
        console.log('[FactoryGuard] 📋 Topic:', testTopic)
        console.log('[FactoryGuard] 📋 Data:', JSON.stringify(testData, null, 2))
        alert('✅ MQTT Test Successful! Check console for details.')
      } else {
        console.error('[FactoryGuard] ❌ Test message publish failed')
        alert('❌ MQTT publish failed - check console for details')
      }
    } catch (error) {
      console.error('[FactoryGuard] ❌ Test MQTT publish error:', error)
      alert('❌ MQTT test error - check console for details')
    }
  }
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">
            Monitor FactoryGuard AI's automatic connection to supOS Community Edition
          </p>
        </div>
        <div className="flex items-center gap-2">
          <Badge variant="outline" className="text-xs">
            Auto-connected on login
          </Badge>
          <Button
