...
export function getSupOSStatus() {
  // Since data is flowing successfully, consider the connection active
  // The MQTT connection is working even if the initial auth check failed
  const isActuallyConnected = suposConnected || (suposEquipmentData !== null && suposSensorData !== null)

  return {
    connected: isActuallyConnected,
    hasToken: suposToken !== null,
    eventFlowConnected: eventFlowConnection?.readyState === 1 || isActuallyConnected, // Consider MQTT connection active if data is flowing
